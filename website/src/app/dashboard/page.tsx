...
const MOCK_MATCHES = [
  ...
  { game: 'Quantum Chess', time: '2m ago', result: 'lost' as const, amount: 80 },
  { game: 'BlockBlitz', time: '2m ago', result: 'won' as const, amount: 95 },
];
const CATEGORIES = ['All', 'Strategy', 'Puzzle', 'Arcade', 'Board', 'Action'];
const GAMES = [
  { name: 'Quantum Chess', category: 'Strategy', players: '1,200', rating: 4.9, image: '/images/game-quantumchess-banner.png' },
  { name: 'Satoshi Runner', category: 'Action', players: '1,920', rating: 4.4, image: '/images/game-satoshirunner-banner.png' },
  { name: 'BlockBlitz', category: 'Puzzle', players: '850', rating: 4.6, image: '/images/game-blockblitz-banner.png' },
];
      ...
      <div className="px-4 md:px-8 py-6 max-w-7xl mx-auto space-y-6">
          ...
          <div className="rounded-3xl border border-[#7135DB]/40 bg-[#0B1121] p-6 flex flex-col sm:flex-row gap-6 items-center">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-400 mt-2 leading-relaxed">
                Classic arcade snake game on EVM blockchain. Real-time stakes and deterministic physics engine. Last snake standing wins the pool.
              </p>
              <div className="flex items-center gap-3 mt-5">
                <button type="button" className="h-10 px-5 rounded-xl bg-[#169EFA] hover:opacity-90 text-black text-sm font-bold transition-all">
                  Play Now
                </button>
                <button type="button" className="h-10 px-5 rounded-xl border border-white/15 hover:border-white/30 text-sm font-bold text-gray-200 transition-all">
                  Learn More
                </button>
              </div>
            </div>
            <div className="w-full sm:w-64 shrink-0 rounded-2xl overflow-hidden border border-[#7135DB]/30">
              <Image src="/images/game-snake-featured.png" alt="Snake Battle" width={340} height={310} className="w-full h-auto" />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <ProgressCard name="Snake Battle" subtitle="Tournament Arena" percent={75} image="/images/game-snake-card.png" />
            <ProgressCard name="BlockBlitz" subtitle="Tournament Arena" percent={40} image="/images/game-blockblitz-card.png" />
          </div>
            ...
            <div className="space-y-4">
              {MOCK_MATCHES.map((m, i) => (
                <div key={i} className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-black ${m.result === 'won' ? 'text-green-400' : 'text-red-400'}`}>
                    {m.result === 'won' ? 'You Won' : 'You Lost'} {m.result === 'won' ? '+' : '-'}{m.amount} GVT
                  </span>
                </div>
              ))}
            </div>
          ...
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`shrink-0 h-8 px-4 rounded-full text-xs font-bold transition-colors ${
                activeCategory === cat ? 'bg-[#7135DB] text-white' : 'bg-[#0B1121] border border-white/10 text-gray-400 hover:text-white'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="space-y-6">
          {filteredGames.map((game) => (
            <div key={game.name} className="rounded-3xl border border-white/10 bg-[#0B1121] overflow-hidden">
              <Image src={game.image} alt={game.name} width={1100} height={370} className="w-full h-auto" />
              <div className="p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-black text-white">{game.name}</p>
                    <p className="text-sm text-[#22D3EE] mt-0.5">{game.category}</p>
                    <p className="text-xs text-gray-500 mt-0.5">{game.players} players online</p>
                  </div>
                  <span className="flex items-center gap-1 text-sm font-bold text-amber-400 shrink-0">★ {game.rating}</span>
                </div>
...
function StatCard({
  label, value, sub, icon, valueClassName = 'text-white',
}: { label: string; value: string; sub: string; icon: string; valueClassName?: string }) {
  return (
...
function ProgressCard({
  name, subtitle, percent, image,
}: { name: string; subtitle: string; percent: number; image: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0B1121] p-4 flex items-center gap-4">
      <div className="w-16 h-20 rounded-lg overflow-hidden shrink-0">
        <Image src={image} alt="" width={64} height={80} className="w-full h-full object-cover" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-bold text-white truncate">{name}</p>
        <p className="text-xs text-gray-500">{subtitle}</p>
      </div>
      <span className="text-sm font-black text-[#22D3EE] shrink-0">{percent}%</span>
    </div>
  );
}
