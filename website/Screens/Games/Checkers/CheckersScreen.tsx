...
// ── Move generation ──────────────────────────────────────────────────────────
function jumpDirs(): [number, number][] {
  return [
    [-1, -1],
    [-1, 1],
    [1, -1],
...
function normalMoves(b: Board, r: number, c: number, piece: Piece): Move[] {
  return moveDirs(piece)
    ...
    .map(([nr, nc]) => {
      const promotes =
        piece.type === "man" && (piece.color === "red" ? nr === 0 : nr === 7);
      return {
        from: [r, c] as Sq,
        to: [nr, nc] as Sq,
        captured: [] as Sq[],
...
function drawCanvas(
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      ...
      ctx.beginPath();
      ctx.arc(x + 3, y + 3, rad, 0, Math.PI * 2);
      ctx.fill();
      ...
      if (piece.color === "red") {
        ...
        g.addColorStop(0, "#44475a");
        g.addColorStop(1, "#1a1b23");
      }
      ctx.fillStyle = g;
      ctx.beginPath();
...
export default function CheckersScreen() {
  ...
  const fetchMatchState = useCallback(async () => {
    ...
    if (data && user) {
      ...
      const oppId =
        data.player1_id === user.id ? data.player2_id : data.player1_id;
  ...
  useEffect(() => {
    if (mode === "1v1" && matchId) {
      ...
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            setTurn(
              payload.new.current_turn === payload.new.player1_id
                ? "red"
                : "black",
            );
            ...
            if (payload.new.status === "finished") {
              const winColor =
                payload.new.winner_id === payload.new.player1_id
                  ? "red"
                  : "black";
              setStatus("finished");
              setWinner(winColor);
      ...
      const tickTimer = setInterval(async () => {
        await supabase.functions.invoke("game_tick", {
  ...
  useEffect(() => {
    if (canvasRef.current)
      drawCanvas(canvasRef.current, board, selected, highlights, playerColor);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (move) {
        ...
        setBoard(nb);
        setMoveCount((m) => m + 1);
        const followUp = move.captured.length
          ? exploreCaptures(
              nb,
              move.to[0],
              move.to[1],
              nb[move.to[0]][move.to[1]]!,
              [],
            )
          : [];
        if (followUp.length) {
    ...
    async (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const finishMove = async (
        ...
        if (mode === "1v1") {
          ...
          if (user && matchId) {
            const followUp = moveObj.captured.length
              ? exploreCaptures(nb, finalR, finalC, nb[finalR][finalC]!, [])
              : [];
            const isFinished =
              !followUp.length && checkEnd(nb, oppColor(playerColor));
            ...
            if (moveError) {
              console.error(
                "[Checkers] perform_game_move failed:",
                moveError.message,
              );
            }
      ...
      const globalCaptures = allPlayerMoves.filter(
        (m) => m.captured.length > 0,
      );

      const chain = mustContinueRef.current;
      if (chain) {
        const move = highlightsRef.current.find(
          (m) => m.to[0] === r && m.to[1] === c,
        );
        if (move) {
    ...
    return (
      <div className="min-h-screen bg-[#0B1121] flex flex-col items-center justify-center p-4">
        ...
        <Link
          ...
          className="px-6 py-2 rounded-lg bg-red-600 text-white font-bold"
        >
          Back to Games
        </Link>
      </div>
    ...
    <div className="min-h-screen bg-[#0B1121] text-white">
      ...
      <div className="flex flex-col items-center pt-28 pb-16 px-4 gap-6">
        ...
        <div className="flex items-center gap-4 w-full max-w-[600px]">
          ...
          <span
            className={`ml-auto border text-[11px] font-black uppercase px-3 py-0.5 rounded-full ${
              mode === "1v1"
        ...
        {mode === "1v1" && (
          <div className="grid grid-cols-2 gap-4 w-full max-w-[600px] bg-[#0d1326] border border-white/5 p-4 rounded-xl">
            <div
              className={`flex flex-col gap-1 border-r border-white/5 ${turn === playerColor ? "opacity-100" : "opacity-40"}`}
            >
              ...
              <div className="flex items-center justify-between pr-4">
                <div
                  className={`w-4 h-4 rounded-full ${playerColor === "red" ? "bg-red-500" : "bg-gray-700"}`}
                ></div>
                <span
                  className={`text-xl font-mono font-bold ${(playerColor === "red" ? p1Time : p2Time) < 60 ? "text-red-500 animate-pulse" : "text-gray-400"}`}
                >
                  {Math.floor((playerColor === "red" ? p1Time : p2Time) / 60)}:
                  {((playerColor === "red" ? p1Time : p2Time) % 60)
                    .toString()
                    .padStart(2, "0")}
                </span>
              </div>
            ...
            <div
              className={`flex flex-col gap-1 pl-2 ${turn !== playerColor ? "opacity-100" : "opacity-40"}`}
            >
              <span className="text-[10px] font-black uppercase text-gray-500">
              ...
              <div className="flex items-center justify-between">
                <div
                  className={`w-4 h-4 rounded-full ${playerColor === "red" ? "bg-gray-700" : "bg-red-500"}`}
                ></div>
                <span
                  className={`text-xl font-mono font-bold ${(playerColor === "red" ? p2Time : p1Time) < 60 ? "text-red-500 animate-pulse" : "text-gray-400"}`}
                >
                  {Math.floor((playerColor === "red" ? p2Time : p1Time) / 60)}:
                  {((playerColor === "red" ? p2Time : p1Time) % 60)
                    .toString()
                    .padStart(2, "0")}
                </span>
              </div>
        ...
        {status === "finished" && mode === "Solo" && (
          <div className="flex gap-3">
            <button
              onClick={reset}
              className="px-6 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-bold transition-all active:scale-95"
            >
              Play Again
            </button>
            <Link
              href="/dashboard/games"
              className="px-6 py-2.5 rounded-xl border border-white/10 hover:border-white/30 text-white font-bold transition-all"
            >
              Back to Games
            </Link>
          </div>
        ...
        <div className="w-full max-w-[600px] bg-[#0d1326] border border-white/5 rounded-2xl p-8 mt-12 animate-fade-in">
          <h3 className="text-sm font-black text-white uppercase tracking-widest mb-6 flex items-center gap-2">
            <span className="w-6 h-6 rounded-full bg-red-500/10 flex items-center justify-center text-red-500 text-[10px]">
              ?
            </span>
            Game Rules
          </h3>
          ...
          <div className="space-y-6">
            <div className="flex gap-4">
              ...
              <div>
                <h4 className="text-[11px] font-black text-red-500 uppercase mb-1">
                  Winning Conditions
                </h4>
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 leading-relaxed font-medium italic">
                  If you can jump over an opponent's piece to capture it, you
                  MUST do so. This is a standard rule of competitive checkers.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 leading-relaxed font-medium">
                  When a piece reaches the opposite side, it becomes a King and
            ...
            <div className="flex gap-4 pt-4 border-t border-white/5">
              ...
              <div>
                <h4 className="text-[11px] font-black text-yellow-500 uppercase mb-1">
                  1v1 Timing
                </h4>
                <p className="text-xs text-gray-400 leading-relaxed font-medium">
                  Each player has 10 minutes total. Running out of time results
