...
function classifyTx(tx: any, address: string): TxItem {
  ...
  if (to === ESCROW_ADDRESS.toLowerCase()) {
    type = "deposit";
    label = "Deposit";
  }
  if (from === ESCROW_ADDRESS.toLowerCase()) {
    type = "reward";
    label = "Reward";
  }
  if (isOut && to !== ESCROW_ADDRESS.toLowerCase()) {
    type = "withdrawal";
    label = "Withdrawal";
  }
...
export default function WalletScreen() {
  ...
  const fetchTxHistory = useCallback(async () => {
    ...
    try {
      ...
      const [normalRes, tokenRes] = await Promise.all([
        fetch(
          `${base}?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&page=1&offset=10&sort=desc&apikey=${ETHERSCAN_API_KEY}`,
        ),
        fetch(
          `${base}?module=account&action=tokentx&contractaddress=${GVT_ADDRESS}&address=${address}&page=1&offset=10&sort=desc&apikey=${ETHERSCAN_API_KEY}`,
        ),
      ]);
      ...
      if (tokenData.status === "1" && Array.isArray(tokenData.result)) {
        all.push(
          ...tokenData.result.map((tx: any) => ({ ...tx, _isToken: true })),
        );
      }
      if (normalData.status === "1" && Array.isArray(normalData.result)) {
        ...
        const relevant = normalData.result.filter((tx: any) =>
          [GVT_ADDRESS.toLowerCase(), ESCROW_ADDRESS.toLowerCase()].includes(
            (tx.to || "").toLowerCase(),
          ),
        );
  ...
  return (
    <div className="min-h-screen bg-[#050B18] text-white selection:bg-olos-blue/30 overflow-x-hidden pb-24 md:pb-12">
      ...
      <main className="px-4 md:px-8 max-w-[1200px] mx-auto animate-fade-in">
        {showDeposit ? (
          ...
          <>
            <h1 className="text-4xl font-black text-olos-blue tracking-tight mb-8">
              Wallet
            </h1>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${isConnected ? "bg-green-400 animate-pulse" : "bg-olos-blue/50"}`}
                    />
                    <span className="text-[9px] font-bold uppercase tracking-widest text-gray-600">
                      {isConnected
                        ? "Live · Sepolia Testnet"
                        : "Platform balance · Supabase"}
                    </span>
                  </div>
                </div>
                <button
                  ...
                  <div>
                    ...
                    {isConnected && address ? (
                      ...
                      <p className="text-sm text-gray-400">
                        Connect MetaMask or WalletConnect to see your live GVT
                        balance and stake in matches
                      </p>
                    )}
                  </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
              ...
              >
                ...
                {!isConnected && (
                  <span className="text-[9px] text-white/40 absolute bottom-2">
                    Connect wallet
                  </span>
                )}
                {isConnected && isWrongNetwork && (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full border flex items-center justify-center ${isConnected && !isWrongNetwork ? "border-black/20" : "border-white/20"}`}
                >
                  <svg
                    width="20"
                    height="20"
              ...
              >
                ...
                {isConnected && (
                  <p className="text-[9px] text-olos-blue/60 font-bold">
                    Etherscan ↗
                  </p>
                )}
              </a>
              ...
              <div className="bg-[#0B1121]/60 border border-white/10 rounded-[20px] p-6 mb-6">
                ...
                <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 mb-4">
                  <p className="text-xs text-yellow-400 font-bold leading-relaxed">
                    ⚠ Escrow withdrawals require a backend-signed release —
                    coming in the next release. GVT not yet staked stays in your
                    wallet and can be transferred freely.
                  </p>
                </div>
                <input
                  type="number"
                ...
                <button
                  disabled
                  className="w-full py-3 bg-white/5 border border-white/10 rounded-xl text-sm font-black uppercase tracking-widest opacity-40 cursor-not-allowed"
                >
                  Coming Soon
                </button>
              </div>
...
function TxStatusBar({
  ...
  return (
    <div
      className={`rounded-xl px-4 py-3 mb-4 text-xs font-bold flex items-center gap-2
      ${
        status === "success"
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span className="text-[10px] text-olos-blue/50 group-hover:text-olos-blue transition-colors">
              ↗ tx
            </span>
          </div>
        </div>
      ...
      <div className="text-right">
        <p
          className={`text-sm font-black ${tx.isPositive ? "text-green-500" : "text-red-500"}`}
        >
          {tx.amount}
        </p>
...
function MobileNavItem({
  ...
  return (
    ...
    >
      ...
      {active && (
        <div className="w-1 h-1 rounded-full bg-olos-blue mt-0.5 shadow-[0_0_8px_#3B82F6]" />
      )}
    </button>
  );
}
