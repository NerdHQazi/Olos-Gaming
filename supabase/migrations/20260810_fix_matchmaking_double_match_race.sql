...
BEGIN
    ...
    -- Serialize queue/match decisions per game+stake bucket to prevent double-match races.
    PERFORM pg_advisory_xact_lock(hashtext('find_opponent:' || p_game_type || ':' || p_stake_amount::TEXT));
    ...
    IF v_opponent_record.id IS NOT NULL THEN
        ...
        -- Defensive check in case the same two players were already matched in this lock window.
        SELECT id INTO v_match_id
        FROM public.matches
        WHERE status = 'active'
          AND game_type = p_game_type
